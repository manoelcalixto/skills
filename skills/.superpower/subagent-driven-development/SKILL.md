...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Mark task complete via update_plan" [shape=box];
    }

    "Read plan, extract all tasks with full text, note context, seed update_plan" [shape=box];
    "More tasks remain?" [shape=diamond];
    "Dispatch final code reviewer subagent for entire implementation" [shape=box];
    "Use $finishing-a-development-branch" [shape=box style=filled fillcolor=lightgreen];

    "Read plan, extract all tasks with full text, note context, seed update_plan" -> "Dispatch implementer subagent (references/implementer-prompt.md)";
    ...
    "Code quality reviewer subagent approves?" -> "Mark task complete via update_plan" [label="yes"];
    "Mark task complete via update_plan" -> "More tasks remain?";
...
[Seed `update_plan` with all tasks]
